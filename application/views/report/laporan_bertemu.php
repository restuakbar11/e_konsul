<?php
        ...
        $pdf->Cell(60);
        $pdf->Cell(0,10,'Nomor :'.$nomor_urut.'/'.$nomor,0,1,'L');
        ...
        if($pdf->GetStringWidth($teks) < 160){
            ...
            if($text_length < 115){
                ...
                $pdf->Cell(10);
                $pdf->MultiCell(0,6,'Inspektorat Kabupaten Demak  melalui Irban Wilayah '.$irban.' telah menerima konsultasi online Dinas/ Instansi/ Lembaga ' .$opd. ' untuk mendapatakan pelayanan berupa  konsultasi (Consulting)  dengan hasil sebagai :',0,'J');
            }else{
                $pdf->MultiCell(0,6,$teks2,0,'J');
                $pdf->Cell(10);
                $pdf->MultiCell(0,6,'kami Inspektorat Kabupaten Demak  melalui Irban Wilayah '.$irban.' telah menerima konsultasi online Dinas/ Instansi/ Lembaga ' .$opd. ' untuk mendapatakan pelayanan berupa  konsultasi (Consulting)  dengan hasil sebagai :' ,0,'J');   
            }
        }
        ...
        $pdf->Cell(15);
        $pdf->SetFont('times','','12');
        $pdf->Cell(0,10,$permasalahan_name,0,1,'L'); // ENTER
        $pdf->Cell(10);
        $pdf->SetFont('times','B','12');
        ...
        $header = array('No.', 'Nama', 'Jabatan');
            ...
            for($i=0;$i<count($header);$i++)
                ...
                $pdf->Cell($w[0],7,'1',1,0,'C');
                $pdf->Cell($w[1],7,$opd1,1,0,'L');
                $pdf->Cell($w[2],7,'',1,0,'L');
                $pdf->Ln();
                $pdf->Cell(20);
                $pdf->Cell($w[0],7,'2',1,0,'C');
                $pdf->Cell($w[1],7,$opd2,1,0,'L');
                $pdf->Cell($w[2],7,'',1,0,'L');
                $pdf->Ln();
                $pdf->Cell(20);
                $pdf->Cell($w[0],7,'3',1,0,'C');
                $pdf->Cell($w[1],7,$opd3,1,0,'L');
                $pdf->Cell($w[2],7,'',1,0,'L');
        $pdf->Ln();
        $pdf->Cell(15);
        $pdf->SetFont('times','','12');
        $pdf->Cell(0,10,'2. Inspektorat Kabupaten Demak',0,1,'L');
            ...
            for($i=0;$i<count($header);$i++)
                ...
                $pdf->SetFont('times','','10');
                $pdf->Cell($w[0],7,'1',1,0,'C');
                $pdf->Cell($w[1],7,$anggota1,1,0,'L');
                $pdf->Cell($w[2],7,$jabatan1,1,0,'L');
                $pdf->Ln();
                $pdf->Cell(20);
                $pdf->Cell($w[0],7,'2',1,0,'C');
                $pdf->Cell($w[1],7,$anggota2,1,0,'L');
                $pdf->Cell($w[2],7,$jabatan2,1,0,'L');
                $pdf->Ln();
                $pdf->Cell(20);
                $pdf->Cell($w[0],7,'3',1,0,'C');
                $pdf->Cell($w[1],7,$anggota3,1,0,'L');
                $pdf->Cell($w[2],7,$jabatan3,1,0,'L');
        $pdf->Ln();
        ...
        $pdf->Cell(15);
        $pdf->Cell(0,6,'1. '.$uraian_masalah,0,1,'L');
        $pdf->Cell(15);
        ...
        $pdf->Cell(10);
        $pdf->SetFont('times','','12');
        $pdf->Cell(88,6,'('.$ttd_opd.')',1,0,'C'); 
        $pdf->Cell(88,6,'('.$ttd_irban.')',1,1,'C');       
        $pdf->Output();
